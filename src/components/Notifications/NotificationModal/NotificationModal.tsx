...
const NotificationModal: FC<Props> = ({ notification, onClose }) => {
  ...
  useEffect(() => {
    ...
    setFormattedDate(date.toLocaleString('es-MX', {
      hour: '2-digit',
      minute: '2-digit',
      day: '2-digit',
      month: 'short',
  ...
  return (
    <div className="absolute left-1/2 top-20 transform -translate-x-1/2 z-50 bg-white shadow-xl border border-gray-200 rounded-lg w-[90%] max-w-2xl p-6">
      <div className="relative">
        ...
        {notification.recurrenceEndDate && (
          <p className="text-sm text-gray-500 mt-2">
            Finaliza el {new Date(notification.recurrenceEndDate).toLocaleDateString('es-MX')}
          </p>
        )}
      </div>
    </div>
